...
/*
	incrementHeight(event){
		event.preventDefault();
		if(this.state.settings.height < 16){
			let settings = this.state.settings;
	...
	incrementWidth(event){
		event.preventDefault();
		if(this.state.settings.width < 8){
			let settings = this.state.settings;
	...
	decrementHeight(event){
		event.preventDefault();
		if(this.state.settings.height > 2){
			let settings = this.state.settings;
			settings[height] = settings[height] - 1;
			this.setState({settings:settings});
		}
	...
	decrementWidth(event){
		event.preventDefault();
		if(this.state.settings.width > 1){
			let settings = this.state.settings;
			settings[width] = settings[width] - 1;
			this.setState({settings:settings});			
		}
